  ...
  try {
    ...
    if (user.loginLockout && user.lastLoginAttempt) {
      ...
      if (timeSinceLastAttempt < 24 * 60 * 60 * 1000) {
        const lockTimeRemaining = 24 * 60 * 60 * 1000 - timeSinceLastAttempt; // Remaining lockout time
    ...
    // If lockout period is over, reset lockout status and login attempts
    ...
    if (!isPasswordMatch) {
      ...
      if (user.loginAttempts >= MAX_LOGIN_ATTEMPTS) {
        user.loginLockout = true; // Set lockout to true
        await user.save();
        return res.status(403).json({
    ...
    // Successful login
    user.loginAttempts = 0;
    user.loginLockout = false; // Ensure lockout is reset on successful login
